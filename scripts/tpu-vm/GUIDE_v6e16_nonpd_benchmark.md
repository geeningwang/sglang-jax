...
for ep in json.load(sys.stdin).get('networkEndpoints', []):
    print(ep['ipAddress'])
"
# 10.202.0.118   ← worker 0 (use this as WORKER0_IP in the script)
...
WORKER0_IP="10.202.0.118"                                 # internal IP of worker 0
```
...
2. Patches `mesh_utils.py` in-place: reverts `jax.local_devices()` → `jax.devices()` for the
   multi-host TP=16 case (the branch uses `local_devices()` for the GKE 1P1D path; raw TPU
   multi-host needs `devices()`)
3. Checks whether `/tmp/miniconda3/bin/python3.12` exists; installs Miniconda if not
...
```
--nnodes 4  --node-rank {0..3}  --dist-init-addr 10.202.0.118:8088
```

Internally, the server calls `jax.distributed.initialize(coordinator_address="10.202.0.118:8088")`.
