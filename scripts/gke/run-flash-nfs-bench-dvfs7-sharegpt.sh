#!/usr/bin/env bash
# run-flash-nfs-bench-dvfs7-sharegpt.sh — MiMo-V2-Flash MTP benchmark, ShareGPT dataset
#
# Identical to run-flash-nfs-bench-dvfs7-mtp2.sh except:
#   - Job/VM/flag names use the -dvfs7-sharegpt suffix
#   - bench_serving uses --dataset-name sharegpt (natural prompt lengths, no 16K padding)
#   - Results go to: gs://jingnw-mimo-v2-flash-us-central1/perf-results/flash-1node-nfs-tpu7-dvfs7-sharegpt/
#
# Purpose: measure realistic MTP acceptance rate (~65-75%) using natural ShareGPT
# conversation lengths, vs ~98% seen with the random (padded) dataset.

set -euo pipefail

PROJECT="tpu-launchpad-playground"
VM_NAME="nfs-flash-dvfs7-sharegpt"
JOB_NAME="mimo-v2-flash-1node-nfs-tpu7-dvfs7-sharegpt"
READY_FLAG="gs://jingnw-mimo-v2-flash-us-central1/nfs-flash-dvfs7-sharegpt-ready"
NFS_IP_FLAG="gs://jingnw-mimo-v2-flash-us-central1/nfs-flash-dvfs7-sharegpt-ip"
...
kubectl apply -f "${SCRIPT_DIR}/mimo-v2-flash-1node-nfs-tpu7-dvfs7-sharegpt.yaml"
...
echo "${INTERNAL_IP}" | gsutil cp - \
  gs://jingnw-mimo-v2-flash-us-central1/nfs-flash-dvfs7-sharegpt-ip

echo "ready at $(date -u) ip=${INTERNAL_IP}" | gsutil cp - \
  gs://jingnw-mimo-v2-flash-us-central1/nfs-flash-dvfs7-sharegpt-ready
...
STARTUP_SCRIPT
  ...
  for ZONE in us-central1-c us-central1-f us-central1-ai1a; do
    ...
    echo "${STARTUP}" > /tmp/nfs-flash-dvfs7-sharegpt-startup.sh
    if gcloud compute instances create "${VM_NAME}" \
      ...
      --network=default \
      --subnet=default \
      --metadata-from-file=startup-script=/tmp/nfs-flash-dvfs7-sharegpt-startup.sh \
      --quiet 2>&1; then
      CREATED_ZONE="${ZONE}"
...
log ""
log "=== Flash dvfs7 ShareGPT benchmark complete ==="
log "Results: gs://jingnw-mimo-v2-flash-us-central1/perf-results/flash-1node-nfs-tpu7-dvfs7-sharegpt/"
log "  MTP bench: .../mtp/bs{32,64,128}/bench.log"
log "  Server log: .../mtp/server.log"
