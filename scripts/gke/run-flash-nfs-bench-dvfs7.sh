#!/usr/bin/env bash
# run-flash-nfs-bench-dvfs7.sh — MiMo-V2-Flash NFS benchmark with DVFS P-state 7
#
# Identical to run-flash-nfs-bench.sh except:
#   - Job/VM/flag names use the -dvfs7 suffix
#   - LIBTPU_INIT_ARGS="--xla_tpu_dvfs_p_state=7" is set in the pod env (yaml)
#   - Results go to: gs://jingnw-mimo-v2-flash-us-central1/perf-results/flash-1node-nfs-tpu7-dvfs7/
#
# Purpose: measure throughput gain from locking the TPU clock to maximum P-state,
# preventing DVFS frequency scaling between decode steps.

set -euo pipefail

PROJECT="tpu-launchpad-playground"
VM_NAME="nfs-flash-dvfs7"
JOB_NAME="mimo-v2-flash-1node-nfs-tpu7-dvfs7"
READY_FLAG="gs://jingnw-mimo-v2-flash-us-central1/nfs-flash-dvfs7-ready"
NFS_IP_FLAG="gs://jingnw-mimo-v2-flash-us-central1/nfs-flash-dvfs7-ip"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

log() { echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] $*"; }

# ── Step 0: Recreate SSH firewall rules (auto-removed by env) ─────────────────
...
kubectl apply -f "${SCRIPT_DIR}/mimo-v2-flash-1node-nfs-tpu7-dvfs7.yaml"
...
echo "${INTERNAL_IP}" | gsutil cp - \
  gs://jingnw-mimo-v2-flash-us-central1/nfs-flash-dvfs7-ip

echo "ready at $(date -u) ip=${INTERNAL_IP}" | gsutil cp - \
  gs://jingnw-mimo-v2-flash-us-central1/nfs-flash-dvfs7-ready
...
STARTUP_SCRIPT
  ...
  for ZONE in us-central1-c us-central1-f us-central1-ai1a; do
    ...
    echo "${STARTUP}" > /tmp/nfs-flash-dvfs7-startup.sh
    if gcloud compute instances create "${VM_NAME}" \
      ...
      --network=default \
      --subnet=default \
      --metadata-from-file=startup-script=/tmp/nfs-flash-dvfs7-startup.sh \
      --quiet 2>&1; then
      CREATED_ZONE="${ZONE}"
  ...
  log "[nfs-vm] To SSH: gcloud compute ssh ${VM_NAME} --zone=${CREATED_ZONE} --tunnel-through-iap --project=${PROJECT}"
...
log ""
log "=== Flash NFS dvfs7 benchmark complete ==="
log "Results: gs://jingnw-mimo-v2-flash-us-central1/perf-results/flash-1node-nfs-tpu7-dvfs7/"
