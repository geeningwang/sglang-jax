...
VM_NAME="nfs-flash-cp4096"
JOB_NAME="mimo-v2-flash-1node-nfs-tpu7-cp4096"
READY_FLAG="gs://jingnw-mimo-v2-flash-us-central1/nfs-flash-cp4096-ready"
NFS_IP_FLAG="gs://jingnw-mimo-v2-flash-us-central1/nfs-flash-cp4096-ip"
...
kubectl apply -f "${SCRIPT_DIR}/mimo-v2-flash-1node-nfs-tpu7-cp4096.yaml"
...
echo "${INTERNAL_IP}" | gsutil cp - \
  gs://jingnw-mimo-v2-flash-us-central1/nfs-flash-cp4096-ip

# Signal readiness — main script and GKE pod poll for this flag.
echo "ready at $(date -u) ip=${INTERNAL_IP}" | gsutil cp - \
  gs://jingnw-mimo-v2-flash-us-central1/nfs-flash-cp4096-ready
