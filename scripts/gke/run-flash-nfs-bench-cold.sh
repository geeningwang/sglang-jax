#!/usr/bin/env bash
# run-flash-nfs-bench-cold.sh — End-to-end lifecycle for MiMo-V2-Flash NFS benchmark
#
# Flow:
...
VM_NAME="nfs-flash-cold"
JOB_NAME="mimo-v2-flash-1node-nfs-tpu7-cold"
READY_FLAG="gs://jingnw-mimo-v2-flash-us-central1/nfs-flash-cold-ready"
NFS_IP_FLAG="gs://jingnw-mimo-v2-flash-us-central1/nfs-flash-cold-ip"
...
kubectl apply -f "${SCRIPT_DIR}/mimo-v2-flash-1node-nfs-tpu7-cold.yaml"
...
echo "${INTERNAL_IP}" | gsutil cp - \
  gs://jingnw-mimo-v2-flash-us-central1/nfs-flash-cold-ip

# Signal readiness — main script and GKE pod poll for this flag.
echo "ready at $(date -u) ip=${INTERNAL_IP}" | gsutil cp - \
  gs://jingnw-mimo-v2-flash-us-central1/nfs-flash-cold-ready
