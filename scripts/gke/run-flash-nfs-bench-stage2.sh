#!/usr/bin/env bash
# run-flash-nfs-bench-stage2.sh — MiMo-V2-Flash bench on geeningwang/sglang-jax mimo-tpu7-stage2
#
# Identical hardware/server params to stage1 dvfs7 run (§五 in test.md):
#   - NFS tmpfs model weights (weights fully in RAM, no disk I/O during inference)
#   - LIBTPU_INIT_ARGS="--xla_tpu_dvfs_p_state=7" (set in pod yaml)
#   - tp=8 dp=2 ep=8, bsz=32/64/128, random 16384-in/4096-out
#
# Only difference from stage1: clone geeningwang/sglang-jax branch mimo-tpu7-stage2
#
# Results: gs://jingnw-mimo-v2-flash-us-central1/perf-results/flash-1node-nfs-stage2/

set -euo pipefail

PROJECT="tpu-launchpad-playground"
VM_NAME="nfs-flash-stage2"
JOB_NAME="mimo-v2-flash-1node-nfs-stage2"
READY_FLAG="gs://jingnw-mimo-v2-flash-us-central1/nfs-flash-stage2-ready"
NFS_IP_FLAG="gs://jingnw-mimo-v2-flash-us-central1/nfs-flash-stage2-ip"
...
kubectl apply -f "${SCRIPT_DIR}/mimo-v2-flash-1node-nfs-stage2.yaml"
log "Job submitted. DWS may take minutes to hours to provision TPU capacity."

# ── Phase 2: Create NFS VM (parallel with DWS wait) ──────────────────────────
...
echo "${INTERNAL_IP}" | gsutil cp - gs://jingnw-mimo-v2-flash-us-central1/nfs-flash-stage2-ip
echo "ready at $(date -u) ip=${INTERNAL_IP}" | gsutil cp - \
  gs://jingnw-mimo-v2-flash-us-central1/nfs-flash-stage2-ready
...
STARTUP_SCRIPT
  ...
  for ZONE in us-central1-c us-central1-f us-central1-ai1a; do
    ...
    echo "${STARTUP}" > /tmp/nfs-flash-stage2-startup.sh
    if gcloud compute instances create "${VM_NAME}" \
      ...
      --network=default \
      --subnet=default \
      --metadata-from-file=startup-script=/tmp/nfs-flash-stage2-startup.sh \
      --quiet 2>&1; then
      CREATED_ZONE="${ZONE}"
  ...
  if [ -z "${CREATED_ZONE}" ]; then
    log "[nfs-vm] ERROR: All zones exhausted."
    return 1
  fi
...
# ── Phase 3: Wait for DWS provisioning ────────────────────────────────────────
...
log ""
log "=== Flash NFS stage2 benchmark complete ==="
log "Results: gs://jingnw-mimo-v2-flash-us-central1/perf-results/flash-1node-nfs-stage2/"
